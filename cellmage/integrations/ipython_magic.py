...
class IPythonContextProvider(ContextProvider):
        ...
        if _IPYTHON_AVAILABLE and display_object:
            try:
                ...
                if isinstance(display_object, dict) and "accumulated_content" in display_object:
                    ...
                    display_object["display_obj"].update(Markdown(f"**Assistant:**\n{display_object['accumulated_content']}"))
                else:
    ...
    def display_status(self, success: bool, duration: float, tokens_in: int, tokens_out: int, cost_mili_cents: int, model: Optional[str] = None) -> None:
        """
...
class NotebookLLMMagics(Magics):
    ...
    def _get_manager(self) -> ChatManager:
        ...
        try:
            ...
            print("   Please check your configuration (.env file, API keys, directories) and restart the kernel.", file=sys.stderr)
