...
from cellmage.models import Message, PersonaConfig, ConversationMetadata
...
def main():
    ...
    total_tests += 1
    success_count += check(
        "test_persona" in memory_loader.list_personas(),
        "Persona was added successfully"
    )
    ...
    total_tests += 1
    success_count += check(
        "test_snippet" in memory_loader.list_snippets(),
    ...
    success_count += check(
        ...
        "Snippet can be retrieved and has correct content"
    )
    ...
    total_tests += 1
    success_count += check(
        identifier == "test_conversation",
        "Conversation saved with correct identifier"
    )
    ...
    total_tests += 1
    success_count += check(
        len(loaded_messages) == len(messages) and 
        loaded_messages[0].role == "system" and
        loaded_messages[1].role == "user" and
        loaded_messages[2].role == "assistant",
    ...
    success_count += check(
        len(history) == 3,
        "History manager has correct number of messages"
    )
    
    # Test cell ID tracking
    total_tests += 1
    success_count += check(
        "test-cell-id" in history_manager.cell_last_history_index,
        "Cell ID is tracked correctly"
    )
    ...
    total_tests += 1
    success_count += check(
        "test_persona" in personas,
        "Chat manager can list personas"
    )
    ...
    total_tests += 1
    success_count += check(
        llm_client.set_override.called,
        "Setting persona applies configuration overrides"
    )
    ...
    total_tests += 1
    success_count += check(
        "test_snippet" in snippets,
        "Chat manager can list snippets"
    )
    
    chat_manager.add_snippet("test_snippet")
    total_tests += 1
    success_count += check(
        any(m.is_snippet for m in chat_manager.get_history()),
        "Snippet was added to history"
    )
    ...
    success_count += check(
        ...
        "Chat returns the expected response"
    )
    ...
    success_count += check(
        ...
        "LLM client was called during chat"
    )
    ...
    success_count += check(
        len(history) > 0,
        "Messages were added to history during chat"
    )
